<x-app-layout>
    <div class="container max-w-lg p-6 mx-auto bg-white rounded-lg shadow-lg">
        <h1 class="mb-6 text-3xl font-extrabold text-gray-800">Create Role</h1>
        ...
        <form action="{{ route('roles.store') }}" method="POST" class="space-y-4">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="name" id="name" class="block w-full p-2 mt-1 transition border border-gray-300 rounded-lg" value="{{ old('name') }}" required>
                @error('name')
            ...
            <button type="submit" class="px-6 py-3 text-white transition rounded-lg bg-gradient-to-r from-[#f590b0] to-[#f56e98]">Create Role</button>
        </form>
    </div>
</x-app-layout>